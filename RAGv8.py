...
################## initial ##############
    def __init__(self, root):
        self.torch = torch
        self.root = root
        self.root.title("高级PDF RAG系统 v8 (Ollama,lm studio)")
...
#################################### GUI service #########################
    ...
    def update_system_ready_message(self):
        ...
        self.vectorstore_info.set(f"向量库: 已加载 ({len(self.individual_vectorstores)}个)")
    ...
    def send_message(self, event=None):
        ...
        try:
            ...
            if selected_indices:# 如果选中
                if self.last_selection != selected_indices:# 如果选中有变化
                    target_vectors = {key: self.individual_vectorstores[key] for key in (self.pdf_paths[i]  for i in selected_indices)}
                    self.last_selection = selected_indices # 上回选取的pdf
                    target_vectors_merge = self.merge_vectorstores(target_vectors) 
